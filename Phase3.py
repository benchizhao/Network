...
def generate_data_repeatall_fix_points():
    ...
    for i in m:
        ...
        f = open("m=%d,init=20,add=100000,repeat5,phase3.txt" %(i),"wb")
        pickle.dump(k,f)
        f.close()
...
def generate_data_repeat_k():
    ...
    for pt in add_points:
        ...
        f = open("m=6,init=20,add=%d,kmax,phase3.txt" %(pt),"wb")
        pickle.dump(k_max,f)
        f.close()
...
def plot_figure_binned():    # plot binned figure
    ...
    for m_val in m:
        val = pickle.load(open("m=%d,init=20,add=100000,repeat5,phase3.txt"%(m_val), "rb"))
